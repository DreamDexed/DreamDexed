...
constexpr uint16_t CommandWord(const char Command[2]) { return Command[0] << 8 | Command[1]; }
...
bool ParseInvitationPacket(const uint8_t *pBuffer, size_t nSize, TAppleMIDISession *pOutPacket)
{
...
bool ParseEndSessionPacket(const uint8_t *pBuffer, size_t nSize, TAppleMIDISession *pOutPacket)
{
	const TAppleMIDISession *const pInPacket = reinterpret_cast<const TAppleMIDISession *>(pBuffer);
...
bool ParseSyncPacket(const uint8_t *pBuffer, size_t nSize, TAppleMIDISync *pOutPacket)
{
	...
	const uint32_t nSignature = ntohs(pInPacket->nSignature);
	if (nSignature != AppleMIDISignature)
		return false;

	const uint32_t nCommand = ntohs(pInPacket->nCommand);
	if (nCommand != Sync)
		return false;
...
size_t ParseSysExCommand(const uint8_t *pBuffer, size_t nSize, CAppleMIDIHandler *pHandler)
{
...
size_t ParseMIDICommand(const uint8_t *pBuffer, size_t nSize, uint8_t &nRunningStatus, CAppleMIDIHandler *pHandler)
{
...
bool ParseMIDICommandSection(const uint8_t *pBuffer, size_t nSize, CAppleMIDIHandler *pHandler)
{
	...
	uint16_t nMIDICommandLength = nMIDIHeader & 0x0F;
...
bool ParseMIDIPacket(const uint8_t *pBuffer, size_t nSize, TRTPMIDI *pOutPacket, CAppleMIDIHandler *pHandler)
{
...
bool CAppleMIDIParticipant::SendPacket(CSocket *pSocket, CIPAddress *pIPAddress, uint16_t nPort, const void *pData, size_t nSize)
{
...
bool CAppleMIDIParticipant::SendMIDIToHost(const uint8_t *pData, size_t nSize)
{
