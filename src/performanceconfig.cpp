...
bool CPerformanceConfig::Load (void)
{
	...
	for (unsigned nFX = 0; nFX < CConfig::FXChains; ++nFX)
	{
		...
		PropertyName.Format ("FX%uChorusMix", nFX+1);
		m_nFXChorusMix[nFX] = m_Properties.GetNumber (PropertyName, 0);
		
		PropertyName.Format ("FX%uChorusEnable1", nFX+1);
		m_bFXChorusEnable1[nFX] = m_Properties.GetNumber (PropertyName, 1);

		PropertyName.Format ("FX%uChorusEnable2", nFX+1);
		m_bFXChorusEnable2[nFX] = m_Properties.GetNumber (PropertyName, 1);
		...
		PropertyName.Format ("FX%uReverbMix", nFX+1);
		m_nFXReverbMix[nFX] = m_Properties.GetNumber (PropertyName, 0);
...
bool CPerformanceConfig::Save (void)
{
	...
	for (unsigned nFX = 0; nFX < CConfig::FXChains; ++nFX)
	{
		...
		PropertyName.Format ("FX%uChorusMix", nFX+1);
		m_Properties.SetNumber (PropertyName, m_nFXChorusMix[nFX]);
		...
		PropertyName.Format ("FX%uReverbMix", nFX+1);
		m_Properties.SetNumber (PropertyName, m_nFXReverbMix[nFX]);
...
unsigned CPerformanceConfig::GetFXChorusMix (unsigned nFX) const
{
	assert (nFX < CConfig::FXChains);
	return m_nFXChorusMix[nFX];
}
...
unsigned CPerformanceConfig::GetFXReverbMix (unsigned nFX) const
{
	assert (nFX < CConfig::FXChains);
	return m_nFXReverbMix[nFX];
}
...
void CPerformanceConfig::SetFXChorusMix (unsigned nValue, unsigned nFX)
{
	assert (nFX < CConfig::FXChains);
	m_nFXChorusMix[nFX] = nValue;
}
...
void CPerformanceConfig::SetFXReverbMix (unsigned nValue, unsigned nFX)
{
	assert (nFX < CConfig::FXChains);
	m_nFXReverbMix[nFX] = nValue;
}
