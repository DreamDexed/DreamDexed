	...
	m_bLoadPerformanceBankBusy(false)
{
	...
	for (unsigned nFX = 0; nFX < CConfig::FXChains; nFX++)
	{
		...
		SetFXParameter (FXParameterDelayTone, 80, nFX);
		SetFXParameter (FXParameterDelayPingPong, 0, nFX);
...
void CMiniDexed::SetFXParameter (TFXParameter Parameter, int nValue, unsigned nFX)
{
	...
	case FXParameterDelayTimeR:
		nValue=constrain(nValue,0,100);
		m_FXSpinLock.Acquire ();
		fx_chain[nFX]->delay.setTimeR (nValue / 100.0f);
		m_FXSpinLock.Release ();
		break;
	...
	case FXParameterDelayTone:
		nValue=constrain(nValue,0,100);
		m_FXSpinLock.Acquire ();
		fx_chain[nFX]->delay.setTone (nValue / 100.0f);
		m_FXSpinLock.Release ();
		break;

	case FXParameterDelayPingPong:
		nValue=constrain(nValue,0,1);
		m_FXSpinLock.Acquire ();
		fx_chain[nFX]->delay.setPingPongMode (nValue);
		m_FXSpinLock.Release ();
		break;
...
bool CMiniDexed::DoSavePerformance (void)
{
	...
	for (unsigned nFX = 0; nFX < CConfig::FXChains; ++nFX)
	{
		...
		m_PerformanceConfig.SetFXDelayTone (GetFXParameter (FXParameterDelayTone, nFX), nFX);
		m_PerformanceConfig.SetFXDelayPingPong (GetFXParameter (FXParameterDelayPingPong, nFX), nFX);
...
void CMiniDexed::LoadPerformanceParameters(void)
{
	...
	for (unsigned nFX=0; nFX < CConfig::FXChains; ++nFX)
	{
		...
		SetFXParameter (FXParameterChorusLFORate2, m_PerformanceConfig.GetFXChorusLFORate1(nFX), nFX);
		...
		SetFXParameter (FXParameterDelayTone, m_PerformanceConfig.GetFXDelayTone(nFX), nFX);
		SetFXParameter (FXParameterDelayPingPong, m_PerformanceConfig.GetFXDelayPingPong(nFX), nFX);
