...
m_pUI(pUI)
{
	...
	for (int tg = 0; tg < 8; tg++)
	{
		...
		{
			uint8_t cc = MIDISystemCCMap[m_nMIDISystemCCVol][tg];
			m_MIDISystemCCBitmap[cc >> 5] |= (1 << (cc % 32));
		}
		if (m_nMIDISystemCCPan != 0)
		{
			uint8_t cc = MIDISystemCCMap[m_nMIDISystemCCPan][tg];
			m_MIDISystemCCBitmap[cc >> 5] |= (1 << (cc % 32));
		}
		if (m_nMIDISystemCCDetune != 0)
		{
			uint8_t cc = MIDISystemCCMap[m_nMIDISystemCCDetune][tg];
			m_MIDISystemCCBitmap[cc >> 5] |= (1 << (cc % 32));
		}
	}
...
void CMIDIDevice::SetChannel(uint8_t ucChannel, unsigned nTG)
{
...
uint8_t CMIDIDevice::GetChannel(unsigned nTG) const
{
	assert(nTG < CConfig::AllToneGenerators);
	return m_ChannelMap[nTG];
}

void CMIDIDevice::MIDIMessageHandler(const uint8_t *pMessage, size_t nLength, unsigned nCable)
{
