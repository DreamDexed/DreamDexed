...
const int8_t MIDISystemCCMap[NUM_MIDI_CC_MAPS][8] = {
...
m_pUI{pUI}
{
	...
	for (int tg = 0; tg < 8; tg++)
	{
		...
		{
			int8_t cc = MIDISystemCCMap[m_nMIDISystemCCVol][tg];
			m_MIDISystemCCBitmap[cc >> 5] |= (1 << (cc % 32));
		}
		if (m_nMIDISystemCCPan != 0)
		{
			int8_t cc = MIDISystemCCMap[m_nMIDISystemCCPan][tg];
			m_MIDISystemCCBitmap[cc >> 5] |= (1 << (cc % 32));
		}
		if (m_nMIDISystemCCDetune != 0)
		{
			int8_t cc = MIDISystemCCMap[m_nMIDISystemCCDetune][tg];
			m_MIDISystemCCBitmap[cc >> 5] |= (1 << (cc % 32));
		}
	}
...
void CMIDIDevice::SetChannel(int nChannel, int nTG)
{
	assert(nTG < CConfig::AllToneGenerators);

	// When changing to OMNI mode, store the previous channel
	if (nChannel == OmniMode && m_ChannelMap[nTG] != OmniMode)
	{
		m_PreviousChannelMap[nTG] = m_ChannelMap[nTG];
	}

	m_ChannelMap[nTG] = static_cast<uint8_t>(nChannel);
}

int CMIDIDevice::GetChannel(int nTG) const
{
	assert(nTG < CConfig::AllToneGenerators);
	return m_ChannelMap[nTG];
}

void CMIDIDevice::MIDIMessageHandler(const uint8_t *pMessage, int nLength, int nCable)
{
	...
	if (m_pConfig->GetMIDIDumpEnabled())
	{
		...
		case 1:
			...
			{
				LOGNOTE("MIDI%d: %02X", nCable, (unsigned)pMessage[0]);
			}
			break;

		case 2:
			LOGNOTE("MIDI%d: %02X %02X", nCable,
				(unsigned)pMessage[0], (unsigned)pMessage[1]);
			break;

		case 3:
			LOGNOTE("MIDI%d: %02X %02X %02X", nCable,
				(unsigned)pMessage[0], (unsigned)pMessage[1],
				(unsigned)pMessage[2]);
			break;
		...
		default:
			...
			case MIDI_SYSTEM_EXCLUSIVE_BEGIN:
				LOGNOTE("MIDI%d: SysEx data length: [%d]:", nCable, nLength);
				...
				break;
			default:
				LOGNOTE("MIDI%d: Unhandled MIDI event type %0x02x", nCable, pMessage[0]);
			}
			break;
		}
	}
	...
	/*
		...
		{
			printf("MIDI%d: SysEx data length: [%d]:",nCable, nLength);
			for (int i = 0; i < nLength; i++)
			{
				if((i % 16) == 0)
					printf("\n%04d:",i);
	...
	*/
	...
	uint8_t ucChannel = ucStatus & 0x0F;
	uint8_t ucType = ucStatus >> 4;
	uint8_t ucP1 = pMessage[1];
	uint8_t ucP2 = nLength > 2 ? pMessage[2] : 0xFF;
	    ...
	    pMessage[6] == MIDI_SYSTEM_EXCLUSIVE_END)
	{
		...
		for (int nTG = 0; nTG < m_pConfig->GetToneGenerators(); nTG++)
		{
	...
	// and then split into LSB/MSB.
		 ...
		 pMessage[7] == MIDI_SYSTEM_EXCLUSIVE_END) // MASTER VOLUME
	{
		// Convert LSB/MSB to 14-bit integer volume
		int nMasterVolume = ((pMessage[5] & 0x7F) | ((pMessage[6] & 0x7F) << 7));
		m_pSynthesizer->SetParameter(CMiniDexed::ParameterMasterVolume, mapint(nMasterVolume, 0, 16383, 0, 127));
	}
	else
	{
		// Perform any MiniDexed level MIDI handling before specific Tone Generators
		int nPerfCh = m_pSynthesizer->GetPerformanceSelectChannel();
		switch (ucType)
		{
		case MIDI_CONTROL_CHANGE:
			...
			if (m_nMIDIGlobalExpression != Disabled)
			{
				...
				if (ucChannel == m_nMIDIGlobalExpression)
				{
					...
					for (int nTG = 0; nTG < m_pConfig->GetToneGenerators(); nTG++)
					{
						m_pSynthesizer->SetExpression(ucP2, nTG);
					}
				}
			}
			if (nLength == 3)
		...
		{
			uint8_t ucSysExChannel = (ucP2 & 0x0F);
			for (int nTG = 0; nTG < m_pConfig->GetToneGenerators(); nTG++)
			{
				...
				if (m_pSynthesizer->GetSysExEnable(nTG) && m_pSynthesizer->GetSysExChannel(nTG) == ucSysExChannel)
				{
					...
					if (nLength == 7 && pMessage[3] == 0x04)
					{
						...
						case 26: // Audio Output Level Attenuator
						{
							...
							int newVolume = (attenVal == 0) ? 0 : (127 >> (7 - attenVal));
						...
						case 64: // Master Tuning
							...
							m_pSynthesizer->SetMasterTune(mapint(val, 1, 127, -37, 37), nTG); // Would need 37.5 here, due to wrong constrain on dexed_synth module?
							break;
						default:
		...
		{
			for (int nTG = 0; nTG < m_pConfig->GetToneGenerators() && !bSystemCCHandled; nTG++)
			{
				if (m_ChannelMap[nTG] == ucChannel || m_ChannelMap[nTG] == OmniMode)
				{
					...
					case MIDI_NOTE_ON:
						...
						if (ucP2 > 0)
						{
							if (ucP2 <= 127)
							{
								m_pSynthesizer->keydown(ucP1, ucP2, nTG);
							}
						}
						else
						{
					...
					case MIDI_CONTROL_CHANGE:
						...
						case MIDI_CC_PORTAMENTO_TIME:
							if (m_pSynthesizer->GetTGParameter(CMiniDexed::TGParameterMIDIRxPortamento, nTG))
								m_pSynthesizer->setPortamentoTime(mapint(ucP2, 0, 127, 0, 99), nTG);
							break;
						...
						case MIDI_CC_RESONANCE:
							...
							m_pSynthesizer->SetResonance(mapint(ucP2, 0, 127, 0, 99), nTG);
							break;
						...
						case MIDI_CC_FREQUENCY_CUTOFF:
							...
							m_pSynthesizer->SetCutoff(mapint(ucP2, 0, 127, 0, 99), nTG);
							break;

						case MIDI_CC_EFFECT1_SEND:
							m_pSynthesizer->SetFX1Send(mapint(ucP2, 0, 127, 0, 99), nTG);
							break;

						case MIDI_CC_EFFECT2_SEND:
							m_pSynthesizer->SetFX2Send(mapint(ucP2, 0, 127, 0, 99), nTG);
							break;
						...
						case MIDI_CC_DETUNE_LEVEL:
							...
							else
							{
								...
								m_pSynthesizer->SetMasterTune(mapint(ucP2, 1, 127, -99, 99), nTG);
							}
							break;
						...
						case MIDI_CC_OMNI_MODE_OFF:
							...
							if (m_ChannelMap[nTG] == OmniMode)
							{
								...
								uint8_t channelToRestore = (m_PreviousChannelMap[nTG] != Disabled) ? m_PreviousChannelMap[nTG] : ucChannel;
...
bool CMIDIDevice::HandleMIDISystemCC(const uint8_t ucCC, const uint8_t ucCCval)
{
	...
	for (int tg = 0; tg < 8; tg++)
	{
		...
		if (m_nMIDISystemCCDetune != 0)
		{
			if (ucCC == MIDISystemCCMap[m_nMIDISystemCCDetune][tg])
			{
				...
				{
					// Scale to -99 to +99 cents
					m_pSynthesizer->SetMasterTune(mapint(ucCCval, 1, 127, -99, 99), tg);
				}
				return true;
			}
		}
...
void CMIDIDevice::HandleSystemExclusive(const uint8_t *pMessage, const int nLength, const int nCable, const int nTG)
{
	...
	int16_t sysex_return = m_pSynthesizer->checkSystemExclusive(pMessage, nLength, nTG);
...
void CMIDIDevice::SendSystemExclusiveVoice(int nVoice, const std::string &deviceName, int nCable, int nTG)
{
	...
	if (Iterator != s_DeviceMap.end())
	{
		...
		LOGDBG("Send SYSEX voice dump %d to \"%s\"", nVoice, deviceName.c_str());
	}
	else
	{
