...
AnalogFilter::AnalogFilter(int Ftype, float Ffreq, float Fq, int Fstages, float srate) :
outgain{1.0f},
samplerate{srate},
type{Ftype},
stages{Fstages},
...
void AnalogFilter::singlefilterout(float *smp, fstage &hist, float f, int bufsize)
{
	assert((bufsize % 8) == 0);
...
void AnalogFilter::filterout(float *smp, int period)
{
	int freqbufsize = period / 8;
