	...
	if (m_pConfig->GetLCDEnabled ())
	{
		...
		else if (st7789)
		{
			...
			if (m_pST7789Display->Initialize())
			{
				...
				bool bLargeFont = false; //!(m_pConfig->GetST7789SmallFont());
				m_pST7789 = new CST7789Device (m_pSPIMaster, m_pST7789Display, m_pConfig->GetLCDColumns (), m_pConfig->GetLCDRows (), Font12x22, bLargeFont, bLargeFont);
