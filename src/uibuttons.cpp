...
bool CUIButton::Initialize(unsigned pinNumber, unsigned doubleClickTimeout, unsigned longPressTimeout, unsigned MIDIRelativeDebounceTime)
{
	assert(!m_pin);
...
CUIButtons::CUIButtons(CConfig *pConfig) : m_pConfig(pConfig),
					   m_eventHandler(0),
					   m_lastTick(0)
{
}
...
bool CUIButtons::Initialize(void)
{
	assert(m_pConfig);
