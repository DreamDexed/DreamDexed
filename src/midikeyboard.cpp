...
void CMIDIKeyboard::Send(const uint8_t *pMessage, size_t nLength, unsigned nCable)
{
	TSendQueueEntry Entry;
	Entry.pMessage = new uint8_t[nLength];
...
// before parsing.
void CMIDIKeyboard::USBMIDIMessageHandler(uint8_t *pPacket, unsigned nLength, unsigned nCable, unsigned nDevice)
{
